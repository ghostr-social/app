...
Future<List<NostrEventRecord>> loadNostrCommentEvents(
  ...
  final regular = references.where((event) => event.identifier == null);
  final addressable = references.where((event) => event.identifier != null);
  final batches = <List<NostrEventRecord>>[];
  if (regular.isNotEmpty) {
    batches.add(await loadFairNostrEvents(
      client,
      regular.toList(),
...
NostrEventQuery _regularCommentQuery(
  NostrEventReference reference,
) {
  return _commentQuery('E', reference);
}

NostrEventQuery _addressableCommentQuery(
  NostrEventReference reference,
) {
  return _commentQuery('A', reference);
}

NostrEventQuery _commentQuery(
  String tagName,
  NostrEventReference target,
) {
  return NostrEventQuery(
    ...
    tagFilters: <NostrTagFilter>[
      NostrTagFilter(
        name: tagName,
        values: [nostrCommentRootValue(target)],
      ),
    ],
    limit: 500,
  );
