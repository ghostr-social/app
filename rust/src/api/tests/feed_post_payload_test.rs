...
use crate::api::feed_mapping::{feed_post, post_gateway_id};
...
fn every_row_field_is_carried_over() {
    let row = feed_post(&post(DeliveryKind::Progressive), creator());
    ...
    assert_eq!((row.creator.pubkey, row.creator.display_name), ("a1".to_owned(), "Vera".to_owned()));
    assert_eq!(row.creator.handle, "@npub1vera");
    assert_eq!(row.creator.avatar_url.as_deref(), Some("https://cdn.example/a.png"));
    assert_eq!(row.media.urls, vec!["https://cdn.example/v.mp4".to_owned()]);
    assert_eq!(row.media.delivery, "progressive");
    assert_eq!(row.media.sha256.as_deref(), Some("ff".repeat(32).as_str()));
    assert_eq!((row.media.size_bytes, row.media.duration_ms), (Some(9), Some(2_000)));
    ...
    assert_eq!(row.media.thumb_url.as_deref(), Some("https://cdn.example/t.jpg"));
}

#[test]
fn hls_delivery_round_trips_the_v1_focus_vocabulary() {
    let row = feed_post(&post(DeliveryKind::Hls), creator());
    assert_eq!(row.media.delivery, "hls");
}
