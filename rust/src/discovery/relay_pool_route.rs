...
use crate::discovery::relay_io::{RelayBroadcastIo, RelayReadIo};
use crate::discovery::relay_pool_owner::{
    session_failure, RelayBroadcastRequest, RelayPoolOwner, RelayReadRequest,
};
use crate::discovery::relay_pool_roles::RelayRole;
use std::sync::Arc;
use tokio::sync::{watch, OwnedRwLockReadGuard};

const NO_RELAYS_MESSAGE: &str = "no Nostr relays are configured";

pub(crate) struct RelayPoolRoute {
    owner: RelayPoolOwner,
    session: crate::discovery::session_generation::SessionGeneration,
    epoch: u64,
...
impl RelayPoolOwner {
    pub(crate) async fn begin_route(
        &self,
        session: crate::discovery::session_generation::SessionGeneration,
