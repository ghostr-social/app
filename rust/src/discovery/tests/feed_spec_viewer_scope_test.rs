...
fn a_signed_in_main_feed_scopes_the_pool_to_its_viewer() {
    let spec = FeedSpec::MainFeed { viewer: Some(author(AUTHOR_A)) };
...
fn a_signed_out_main_feed_scopes_the_pool_to_nobody() {
    assert_eq!(scope(&FeedSpec::MainFeed { viewer: None }), ViewerScope::SignedOut);
}

#[test]
fn a_query_feed_leaves_the_scope_alone() {
    assert_eq!(scope(&FeedSpec::Search("skate".into())), ViewerScope::Unknown);
    assert_eq!(scope(&FeedSpec::Hashtag("skate".into())), ViewerScope::Unknown);
    assert_eq!(
