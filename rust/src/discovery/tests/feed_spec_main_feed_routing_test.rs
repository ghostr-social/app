...
//! `authors`. ndk's `forYou` query is unscoped and outbox-routed, so the
//! follows' relays answer with everything they carry.

use crate::discovery::feed_spec::FeedSpec;
use crate::discovery::social_graph::SocialGraph;
use crate::discovery::tests::outbox_support::contact_list_event;
...
fn the_main_feed_routes_by_follows_and_filters_by_nobody() {
    ...
    let request = FeedSpec::MainFeed { viewer: Some(viewer.public_key()) }
        .page_request(None, &graph)
        .expect("the main feed always queries");

    assert_eq!(request.routing_authors, vec![follow]);
    assert!(request.authors.is_empty(), "the main feed filters by nobody");
}
