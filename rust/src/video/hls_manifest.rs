...
use crate::video::hls_manifest_tags::{action, HlsTagAction};
...
where
    Issue: FnMut(HlsResource) -> Result<String>,
{
    let attribute = quoted_attribute(line, "URI")?;
    if required && attribute.is_none() {
        bail!("HLS tag is missing its URI attribute");
    }
    let Some((start, end)) = attribute else {
