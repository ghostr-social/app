...
fn vbr_samples_map_playback_time_to_their_exact_byte_extents() {
    ...
    let first = timeline.media_ranges(PlaybackWindow::try_new(0, 1_000).unwrap());
    let second = timeline.media_ranges(PlaybackWindow::try_new(1_000, 2_000).unwrap());
    let first_two = timeline.media_ranges(PlaybackWindow::try_new(0, 2_000).unwrap());
