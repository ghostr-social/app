...
fn measured_fast_host_outranks_measured_slow_host() {
    ...
    let ordered = stats.best_source(&candidates);
...
fn tied_hosts_keep_their_imeta_order() {
    ...
    assert_eq!(stats.best_source(&candidates), candidates);
}

#[test]
fn health_ranking_drops_a_failing_host_behind_an_unknown_mirror() {
    let mut stats = HostStats::new();
...
#[test]
fn a_failing_host_sinks_behind_a_healthy_mirror() {
    let mut stats = HostStats::new();
    ...
    let ordered = stats.best_source(&candidates);
    assert_eq!(ordered[0], "https://steady.example/v.mp4");
}
