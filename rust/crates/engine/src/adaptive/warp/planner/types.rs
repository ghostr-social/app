...
impl<'a> WarpPlannerInput<'a> {
    ...
    pub(super) fn playback_emergency(&self) -> bool {
        self.snapshot.playback.authority == crate::adaptive::CurrentAuthority::Canonical
            && self
                .snapshot
                .candidates
                .iter()
                .find(|candidate| candidate.post == self.snapshot.playback.current)
                .is_some_and(|candidate| {
                    crate::adaptive::resources::endangered(self.snapshot, candidate)
                })
    }
}
