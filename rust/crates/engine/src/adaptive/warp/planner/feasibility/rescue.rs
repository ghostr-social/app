...
pub(super) fn select(inputs: RescueInputs<'_>) -> Option<RescuePlan> {
    ...
    let current_only = current_recovery_available(&inputs);
    inputs
        .frontier
        .iter()
        .filter(|node| rescue_candidate(&inputs, current_only, node))
        .filter(|node| node.forecast.ready_playback_ms > 0)
        .filter_map(|node| candidate(&inputs, node))
        .min_by_key(plan_key)
}

fn rescue_candidate(inputs: &RescueInputs<'_>, current_only: bool, node: &ActionNode) -> bool {
    !current_only || node.post == inputs.input.snapshot.playback.current
}

fn current_recovery_available(inputs: &RescueInputs<'_>) -> bool {
    inputs.input.playback_emergency()
        && inputs.frontier.iter().any(|node| {
            node.post == inputs.input.snapshot.playback.current
                && node.forecast.ready_playback_ms > 0
                && super::request_admitted(inputs.input, node)
        })
}
