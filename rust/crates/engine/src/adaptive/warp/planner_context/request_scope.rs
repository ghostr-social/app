...
impl PlannerContext {
    pub(crate) fn with_one_additional_request_slot(mut self, ceiling: u16) -> Self {
        self.limits.request_tokens = expanded(self.limits.request_tokens, ceiling);
        if let Some(scope) = &mut self.request_scope {
            scope.ordinary_tokens = expanded(scope.ordinary_tokens, ceiling);
        }
        self
    }
}

fn expanded(current: u16, ceiling: u16) -> u16 {
    current.saturating_add(1).min(ceiling)
}
