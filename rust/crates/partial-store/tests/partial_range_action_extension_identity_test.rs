...
async fn stale_same_id_extension_cannot_shrink_a_new_action() {
    ...
    let stale = fixture.store.reserve_action(&identity, 1, 2).await.unwrap();
    let extension = fixture.store.extend_action(&stale, 4).await.unwrap();
    fixture.store.release_action(&stale).await;
    let current = fixture.store.reserve_action(&identity, 1, 4).await.unwrap();
