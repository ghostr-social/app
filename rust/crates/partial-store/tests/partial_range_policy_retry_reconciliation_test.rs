...
async fn policy_retry_releases_payload_debt_after_a_partial_cleanup() {
    ...
    std::fs::set_permissions(&fixture.root, std::fs::Permissions::from_mode(0o700))
        .expect("fixture");
    tokio::fs::remove_file(&intent).await.expect("fixture");
    tokio::fs::create_dir(&intent).await.expect("fixture");
    ...
    assert!(reopened
        ...
        .await
        .is_err());
    tokio::fs::remove_dir(intent).await.expect("fixture");
    let action = reopened
        .store
