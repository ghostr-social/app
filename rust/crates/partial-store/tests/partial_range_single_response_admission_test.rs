...
async fn single_response_keeps_existing_bytes_until_its_first_write_is_admitted() {
    let root = store_fixture::temp_root("single-response-admission");
    ...
    store.write_range("post", 0, b"kept").await.unwrap();

    assert!(store
        .begin_single_response(&transfer, 7, store_fixture::exact_response(8))
        .await
        .unwrap());
