...
//! kind-10002 for a named set of authors.

use crate::outbox_plans::{author_relay_lists_plan, viewer_lists_plan};
use crate::search_queries::{OutboxLookup, QueryRole, RelayTarget};
...
fn the_viewer_plan_asks_for_the_viewers_own_lists() {
    ...
    let query = plan
        .queries
        .first()
        .expect("the viewer plan runs one query");
