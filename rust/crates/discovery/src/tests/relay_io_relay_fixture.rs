...
pub(crate) async fn relay_serving(event: Event) -> String {
    relay_serving_events(vec![event]).await
}

pub(crate) async fn relay_serving_events(events: Vec<Event>) -> String {
...
async fn serve_client(stream: TcpStream, events: Vec<Event>) {
    ...
    while let Some(Ok(message)) = socket.next().await {
        match message {
            Message::Text(payload) => handle_text(&mut socket, &payload, &events).await,
            Message::Ping(bytes) => {
...
async fn handle_text<S>(socket: &mut S, payload: &str, events: &[Event])
where
    S: futures_util::Sink<Message> + Unpin,
{
    ...
    match message.get(0).and_then(Value::as_str) {
        Some("REQ") => send_query_result(socket, &message, events).await,
        Some("EVENT") => send_acceptance(socket, &message).await,
        _ => {}
    }
}

async fn send_query_result<S>(socket: &mut S, message: &Value, events: &[Event])
where
    S: futures_util::Sink<Message> + Unpin,
{
