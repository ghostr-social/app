...
async fn panicking_transfer_reports_terminal_and_releases_its_reservation() {
    ...
    let identity = binding
        .transfer("https://panic.example/video")
    ...
    let (responses, _response_receiver) =
    ...
    let InternalEvent::Transfer(TransferEvent::ChunkDone(done)) =
        events.recv().await.expect("valid test fixture")
    else {
