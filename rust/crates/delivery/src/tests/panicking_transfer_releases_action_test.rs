...
use crate::manager::transfers::{spawn_chunk, ChunkLaunch, InternalEvent, TransferContext, TransferEvent};
...
async fn panicking_transfer_reports_terminal_and_releases_its_reservation() {
    ...
    let identity = binding.transfer("https://panic.example/video").expect("valid test fixture");
    ...
    let (responses, _response_receiver) = response_open::channel(core::time::Duration::from_secs(1));
    ...
    let InternalEvent::Transfer(TransferEvent::ChunkDone(done)) = events.recv().await.expect("valid test fixture")
    else {
        panic!("chunk terminal event")
    };
    assert!(done.outcome.is_err());
    store.reserve_action(&identity, 2, 4).await.expect("valid test fixture");
