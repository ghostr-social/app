...
async fn skipped_empty_intervals_update_prices_exactly_without_iteration() {
    ...
    assert_eq!(first.price_snapshot.unwrap().prices.network_micros, 1_000);

    tokio::time::advance(Duration::from_millis(500_000_000)).await;
    let skipped = control.feedback(environment);
    let snapshot = skipped.price_snapshot.unwrap();
