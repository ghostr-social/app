...
use crate::delivery_events::{command_channel, DeliveryHandle, PlayerPreparationActorOutcome, PlayerPreparationAttempt, PlayerPreparationAuthority, PlayerPreparationDisposition, PlayerPreparationFollowup, PlayerPreparationObservation, PlayerPreparationReport, PlayerPreparationState};
...
async fn equal_client_attempt_numbers_remain_scoped_to_their_posts() {
    ...
    apply_next(&receiver);
    assert_eq!(first.await.expect("valid test fixture"), PlayerPreparationDisposition::Applied);

    let second = confirmation(&handle, report("second", 1, 2));
    tokio::task::yield_now().await;
    apply_next(&receiver);
    assert_eq!(second.await.expect("valid test fixture"), PlayerPreparationDisposition::Applied);
}
...
fn apply_next(receiver: &crate::delivery_events::CommandReceiver) {
...
fn capability_report(generation: u64) -> PlayerPreparationReport {
    ...
    let attempt = PlayerPreparationAttempt::try_new(generation, 7, 1).expect("valid test fixture");
    let observation =
