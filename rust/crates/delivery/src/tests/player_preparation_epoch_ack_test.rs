...
async fn old_completion_cannot_mutate_a_new_epoch_ledger() {
    ...
    let old_envelope = receiver.try_player_preparation_envelope().unwrap();

    let new = report_with_epoch("new", 8, 1, 1);
    let new_pending = confirmation(&handle, new.clone());
    tokio::task::yield_now().await;
    let new_envelope = receiver.try_player_preparation_envelope().unwrap();
    receiver.complete_player_preparation(old_envelope, PlayerPreparationActorOutcome::Applied);
    assert_eq!(old_pending.await.unwrap(), PlayerPreparationDisposition::Unavailable);
    receiver.complete_player_preparation(new_envelope, PlayerPreparationActorOutcome::Applied);
    assert_eq!(new_pending.await.unwrap(), PlayerPreparationDisposition::Applied);

    assert_eq!(
        confirmation(&handle, new).await.unwrap(),
