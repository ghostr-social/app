use crate::client_capability::{
    ClientCapabilityModel, ClientCapabilityProfile, ClientCapabilityState, ClientCapabilityStatus,
};
...
fn restored_capability_state_is_validated_and_bounded() {
    let records: Vec<_> = (0..140)
        .map(|index| record(&format!("representation-{index}"), vec![index]))
        .chain([record("", Vec::new())])
        .collect();
    ...
    let persisted = serde_json::to_value(model.state()).unwrap();
    assert_eq!(persisted["records"].as_array().unwrap().len(), 128);
    ...
    let unknown =
        ClientCapabilityProfile::try_new("missing", Some("hvc1"), Some((1080, 1920))).unwrap();
    assert_eq!(model.status(9, &unknown), ClientCapabilityStatus::Unknown);
}

fn record(representation: &str, samples: Vec<u64>) -> Value {
    json!({
        "profile": {
...
fn profile(representation: &str) -> ClientCapabilityProfile {
    ClientCapabilityProfile::try_new(representation, Some("avc1"), Some((1080, 1920))).unwrap()
}
