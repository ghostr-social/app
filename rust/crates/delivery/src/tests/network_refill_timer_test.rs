...
async fn refill_wake_is_exact_replaceable_and_cancellable() {
    ...
    let stale = timer.reconcile(Some(110), 100, &events).unwrap();
    timer.reconcile(None, 100, &events);
    let current = timer.reconcile(Some(110), 100, &events).unwrap();

    assert_ne!(stale, current);
    assert!(!timer.finish(stale));
    tokio::time::advance(Duration::from_millis(10)).await;
    let InternalEvent::NetworkRefill(reached) = receiver.recv().await.unwrap() else {
