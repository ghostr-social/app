...
fn subthreshold_hls_object_keeps_success_and_ttfb_without_throughput_evidence() {
    let result = completed(65_535, 101, 100);
    let observation = terminal(input(&result)).observation.unwrap();
...
fn reliable_hls_sample_uses_body_time_after_ttfb() {
    let result = completed(65_536, 101, 100);
    let observation = terminal(input(&result)).observation.unwrap();
...
fn completed(
    ...
    elapsed_ms: u64,
    ttfb_ms: u64,
) -> Result<CompletedObject, crate::segmented::fetch::FetchFailure> {
    Ok(CompletedObject {
        bytes,
