...
async fn manager_learns_throughput_before_the_response_reaches_eof() {
    ...
    assert!(stats.host_throughput(&host_of(&origin).unwrap()).is_some());
    assert!(stats.overall_ttfb().unwrap() >= Duration::from_millis(10));
    assert!(!harness.store.is_complete("current").await.unwrap());
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            if let Ok(json) = tokio::fs::read_to_string(path).await {
                let stats = HostStats::from_json(&json).unwrap();
                if stats
                    .overall_throughput()
