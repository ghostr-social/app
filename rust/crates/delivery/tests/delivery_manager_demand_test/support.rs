...
impl DemandScenario {
    ...
    pub fn assert_demand_plan(&self, plan: &PlanEvidence) {
        ...
        assert_eq!(
            ...
            DEMANDED,
            "blocked read gets its exact allocation"
        );
        assert_eq!(
            allocation.authority,
            PreemptionAuthority::PlaybackCritical,
            "blocked read has playback priority"
        );
        let initial = self.initial.as_ref().expect("initial request");
        let retained = ByteRange::new(initial.range.start, initial.range.end);
        assert!(
            plan.plan.retained.iter().any(|item| {
                item.post.as_str() == POST && item.request.requested_bytes() == retained
            }),
            "existing useful transfer is retained"
        );
        assert!(initial.is_open(), "retained transfer remains open");
    }
    ...
    pub async fn complete_in_priority_order(&mut self) {
        ...
        assert_eq!(
            demanded.range,
            DEMANDED.start..DEMANDED.end,
            "origin serves the demanded interval"
        );
        ...
        assert_eq!(
            missing.expect("ordinary gap"),
            vec![gap],
            "ordinary gaps remain missing"
        );
