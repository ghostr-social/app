...
async fn storage_pressure_evicts_a_far_finalized_video_atomically() {
    ...
    let baseline = harness.handle.latest_plan().map_or(0, |plan| plan.revision);
    assert_eq!(
        harness.handle.update_focus(focus_now(items, 1, 0)),
        FocusAdmission::Accepted
    );
    let applied = wait_for_plan(&harness.handle, baseline, |plan| {
        plan.plan.evictions.iter().any(|eviction| {
            eviction.post.as_str() == "p8" && eviction.range == ByteRange::new(0, 55)
        })
    })
    .await;

    assert_eq!(applied.current, Some(PostId::new("p1")));
    assert_eq!(harness.store.used_bytes().await, 45);
    assert_eq!(
        harness
            .store
            .present_ranges("p1")
