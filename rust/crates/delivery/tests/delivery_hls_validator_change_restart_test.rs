...
use delivery_fixture::bounded_hls_generation::{serve, INIT_BYTES};
...
#[tokio::test]
async fn changed_strong_etag_restarts_the_object_without_poisoning_origin() {
    let (source, requests) = serve().await;
    let mut options = DeliveryOptions::default();
    options.params.chunk_bytes = 256 * 1024;
    let harness = start_harness("hls-validator-change", options);
