...
use delivery_fixture::bounded_hls_generation::{serve_full_unstable, INIT_BYTES};
...
#[tokio::test]
async fn repeated_full_responses_to_continuation_exhaust_the_retry_budget() {
    let (source, requests) = serve_full_unstable().await;
    let mut options = DeliveryOptions::default();
    options.params.chunk_bytes = 256 * 1024;
    let harness = start_harness("hls-range-ignored-repeatedly", options);
