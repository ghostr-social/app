...
async fn derived_fast_start_candidate_waits_for_fresh_exact_player_success() {
    ...
    let current_snapshot = store.media_snapshot("current").await.unwrap();
    let input_snapshot = store.media_snapshot("post").await.unwrap();
    let current_binding = current_snapshot.binding().unwrap().clone();
    let input = input_snapshot.binding().unwrap().clone();
    ...
    let derived = transformed.binding().unwrap().clone();
    report_ready(
        &harness.handle,
