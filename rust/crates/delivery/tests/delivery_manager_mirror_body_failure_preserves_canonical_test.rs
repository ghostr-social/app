...
const SHORT_200: &[u8] = b"HTTP/1.1 200 OK\r\n\
...
async fn mirror_body_failure_preserves_the_canonical_prefix() {
    ...
    let (mirror, request) = raw_http::spawn_raw_server(SHORT_200).await;
    ...
    wait_for_failed_whole(&harness.handle).await;
...
async fn wait_for_failed_whole(handle: &ghostr_delivery::delivery_events::DeliveryHandle) {
    wait_for_history(handle, |history| history.records.iter().any(failed_whole)).await;
}
