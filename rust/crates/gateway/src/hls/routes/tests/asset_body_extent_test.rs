...
async fn uncached_partial_body_must_match_its_declared_extent() {
    ...
    let (underflow_bytes, underflow_failed) =
        observed_body(underflow.result.expect("valid test fixture")).await;
    assert!(underflow_failed);
    assert_eq!(underflow_bytes, 2);

    let overflow = exchange(OVERFLOW.to_vec(), &["bytes=4-7"]).await;
    let (overflow_bytes, overflow_failed) =
        observed_body(overflow.result.expect("valid test fixture")).await;
