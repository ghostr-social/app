...
async fn partial_range_capacity_never_consumes_the_free_space_reserve() {
    ...
    assert_eq!(store.effective_capacity().await, 0, "only the reserve is left");
    let refused = store
    ...
    assert!(exhausted.to_string().contains("space"), "unhelpful: {exhausted}");
    assert_eq!(*fixture.used_bytes.lock().await, 2);
    assert_eq!(store.present_ranges("clip").await.expect("ranges"), vec![0..2]);
