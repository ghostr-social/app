...
async fn retries_a_transient_failure_after_a_bounded_delay() {
    let responses = vec![
    ...
    let directory = temp_directory("ghostr-manager-retry");
    ...
    manager.synchronize_once().await.expect("backoff attempt");
    assert!(downloads
        .lock()
        .await
        .values()
    ...
    manager.synchronize_once().await.expect("retry attempt");
