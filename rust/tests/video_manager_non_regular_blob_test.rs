...
#[tokio::test]
async fn replaces_a_non_regular_active_blob() {
    let response = b"HTTP/1.1 200 OK\r\nContent-Length: 5\r\nConnection: close\r\n\r\nvideo";
    let (url, requests) = spawn_response_sequence(vec![response, response]).await;
    let directory = temp_directory("ghostr-manager-non-regular-blob");
    ...
    tokio::fs::create_dir(&path)
        .await
        .expect("replace with directory");
    tokio::fs::write(path.join("unexpected"), b"bad")
        .await
    ...
    assert_eq!(
        tokio::fs::read(&path).await.expect("replacement blob"),
        b"video"
    );
