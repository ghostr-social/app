...
async fn partial_range_store_reloads_its_contents_at_startup() {
    ...
    assert_eq!(
        store.read_range("done", 0..8).await.expect("completed read"),
        Some(b"headtail".to_vec()),
        "the finished file is reused, not re-downloaded"
    );
    assert_eq!(store.present_ranges("half").await.expect("ranges"), vec![0..4]);
    assert_eq!(
    ...
    assert_eq!(*used_bytes.lock().await, 12, "reading does not double-count");
...
async fn seed(root: &std::path::Path) {
    ...
    store.write_range("done", 0, b"headtail").await.expect("bytes");
