...
fn feed_spec_search_trims_and_lowercases_the_query() {
    let request = search("  Sunset Surf ").page_request(None).expect("request");
...
fn feed_spec_search_with_leading_hash_becomes_a_hashtag_request() {
    let request = search("#Cats").page_request(None).expect("request");
...
fn feed_spec_blank_search_never_queries() {
    assert_eq!(search("   ").page_request(None), None);
}
...
fn feed_spec_lone_hash_stays_a_text_search() {
    ...
    let request = search("#").page_request(None).expect("request");
