...
async fn partial_range_writes_resume_after_the_store_gives_bytes_back() {
    let fixture = paced_store("ghostr-pressure-burst", limits(u64::MAX, 1_000), 5_000, RECHECK);
    let store = &fixture.store;
    store.write_range("cold", 0, &[1; 2_000]).await.expect("cold");
    store.write_range("hot", 0, &[2; 100]).await.expect("hot head");
    ...
    assert_eq!(store.present_ranges("hot").await.expect("ranges"), vec![0..900]);
