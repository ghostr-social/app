...
void main() {
  testWidgets('covered renewal reactivates only the renewed exact asset', (
    tester,
  ) async {
    ...
    await pumpVideoPlayerSurface(
      tester,
      port,
      _request(scope, _firstUrl, _authority(_firstCapability), true),
    );
    await pumpVideoPlayerSurface(
      tester,
      port,
      _request(scope, _renewedUrl, _authority(_renewedCapability), false),
    );
    expect(platform.dataSources, hasLength(1));
    await pumpVideoPlayerSurface(
      tester,
      port,
      _request(scope, _renewedUrl, _authority(_renewedCapability), true),
    );
