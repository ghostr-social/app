...
void main() {
  ...
  test('a failed follow preserves its queried contacts for retry', () async {
    ...
    final target = _profile(testFanPublicKey);
    harness.port.failure = StateError('offline');

    await expectLater(social.follow(target), throwsA(isA<AppFailure>()));
    harness.events.events.clear();
    harness.port.failure = null;
