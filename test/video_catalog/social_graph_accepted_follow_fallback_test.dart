...
void main() {
  test('projects an accepted follow into stale local fallback', () async {
    ...
    expect(await graph.toggleFollow(profile), isTrue);
    remote.loadFailure = const AppFailure('relay offline');

    expect(await graph.loadFollowedProfiles(), {profile});
  });
}
