...
void main() {
  ...
  blocTest<SearchCubit, SearchState>(
    'a partial native failure keeps rows visible and reports retrying',
    build: () => SearchCubit(
    ...
    act: (cubit) async {
      ...
      updates.publish(
        'ghost',
        VideoFeedPage(posts: [visible], nextOlderThan: visible.publishedAt),
    ...
    expect: () => [
      ...
      isA<SearchLoaded>(),
      isA<SearchLoaded>()
          .having((state) => state.videos, 'videos', [visible])
          .having((state) => state.canLoadMore, 'can load more', isFalse)
          .having(
            (state) => state.notice,
            'notice',
