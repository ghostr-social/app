...
public final class VideoPlayerPlugin: NSObject, FlutterPlugin, AVFoundationVideoPlayerApi {
  ...
  private let firstFrameEvents: WarpFirstFrameEventStream
  ...
  init(
    ...
    self.firstFrameEvents = WarpFirstFrameEventStream(binaryMessenger: binaryMessenger)
    super.init()
  }
  ...
  func createPlatformViewPlayer(options params: CreationOptions) throws -> Int64 {
    let prepared = try playerItem(with: params)
  ...
  func createTexturePlayer(options creationOptions: CreationOptions) throws -> TexturePlayerIds {
    let prepared = try playerItem(with: creationOptions)
    let player = makeTexturePlayer(item: prepared.item)
    installFirstFrameCallback(on: player, token: prepared.attemptToken)
    let textureId = textureRegistry.register(player)
    player.setTextureIdentifier(textureId)
    let token = prepared.attemptToken
    let playerId = configurePlayer(player) { [weak self] in
      if let token { self?.firstFrameEvents.cancel(token) }
      self?.textureRegistry.unregisterTexture(textureId)
    }
    return TexturePlayerIds(playerId: playerId, textureId: textureId)
  }

  private func makeTexturePlayer(item: FVPAVPlayerItem) -> FVPTextureBasedVideoPlayer {
  ...
  private func installFirstFrameCallback(
    on player: FVPTextureBasedVideoPlayer,
    token: String?
  ) {
    guard let token else { return }
    player.warpFirstFrameRenderedCallback = { [weak self] in
      self?.firstFrameEvents.report(token)
    }
  }
  ...
  private func playerItem(with options: CreationOptions) throws -> WarpPreparedPlayerItem {
    let parsedHeaders = try parseHeaders(options.httpHeaders)
    let headers = parsedHeaders.httpHeaders
    ...
    return WarpPreparedPlayerItem(
      item: avFactory.playerItem(with: asset),
      attemptToken: parsedHeaders.attemptToken
    )
  }

  private func parseHeaders(_ headers: [String: String]) throws -> WarpPlaybackAttemptHeaders {
    do {
      return try WarpPlaybackAttemptHeaders.parse(headers)
    } catch {
      throw PigeonError(
        code: "video_player",
        message: "Invalid \(WarpPlaybackAttemptHeaders.name) header",
        details: nil
      )
    }
  }
