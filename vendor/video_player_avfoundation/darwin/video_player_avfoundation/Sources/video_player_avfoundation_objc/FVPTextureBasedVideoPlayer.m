...
/// Serializes first-frame callback delivery against disposal.
@property(nonatomic) NSLock *firstFrameCallbackLock;
/// Backing storage guarded by firstFrameCallbackLock.
@property(nonatomic, nullable, copy) void (^firstFrameCallbackStorage)(void);
/// Whether the first newly copied frame has already been reported.
@property(nonatomic) BOOL didReportFirstFrame;

/// Ensures that the frame updater runs until a frame is rendered, regardless of play/pause state.
- (void)expectFrame;
/// Reports a newly copied frame once, while holding the disposal fence.
- (void)reportFirstFrameIfNeeded;
@end
...
- (instancetype)initWithPlayerItem:(NSObject<FVPAVPlayerItem> *)item
  ...
  if (self) {
    ...
    _selfRefresh = true;
    _firstFrameCallbackLock = [[NSLock alloc] init];
...
- (void)setWarpFirstFrameRenderedCallback:(void (^)(void))callback {
  [self.firstFrameCallbackLock lock];
  self.firstFrameCallbackStorage =
      !self.disposed && !self.didReportFirstFrame ? [callback copy] : nil;
  [self.firstFrameCallbackLock unlock];
}

- (void (^)(void))warpFirstFrameRenderedCallback {
  [self.firstFrameCallbackLock lock];
  void (^callback)(void) = [self.firstFrameCallbackStorage copy];
  [self.firstFrameCallbackLock unlock];
  return callback;
}
...
- (void)reportFirstFrameIfNeeded {
  [self.firstFrameCallbackLock lock];
  if (!self.disposed && !self.didReportFirstFrame && self.firstFrameCallbackStorage) {
    self.didReportFirstFrame = YES;
    self.firstFrameCallbackStorage();
    self.firstFrameCallbackStorage = nil;
  }
  [self.firstFrameCallbackLock unlock];
}
